# This Makefile is for the Process::SubProcess extension to perl.
#
...
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Process::SubProcess]
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Process/SubProcess.pm]
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Process/ChildProcess.pm \
	lib/Process/ChildProcessExecutable.pm \
...
INST_LIBDIR      = $(INST_LIB)/Process
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Process
...
TO_INST_PM = lib/Process/ChildProcess.pm \
	lib/Process/ChildProcessExecutable.pm \
	lib/Process/SubProcess.pm \
	lib/Process/SubProcess/Group.pm \
	lib/Process/SubProcess/Pool.pm
...
DISTNAME = Process-SubProcess
DISTVNAME = Process-SubProcess-2.0
...
manifypods : pure_all config  \
	lib/Process/ChildProcess.pm \
	lib/Process/ChildProcessExecutable.pm \
	lib/Process/SubProcess.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/Process/ChildProcess.pm $(INST_MAN3DIR)/Process::ChildProcess.$(MAN3EXT) \
	  lib/Process/ChildProcessExecutable.pm $(INST_MAN3DIR)/Process::ChildProcessExecutable.$(MAN3EXT) \
	  lib/Process/SubProcess.pm $(INST_MAN3DIR)/Process::SubProcess.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: Process-SubProcess' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: '\''2.0'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Process-SubProcess",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "2.0",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Process-SubProcess" VERSION="2.0">' > Process-SubProcess.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> Process-SubProcess.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> Process-SubProcess.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Process-SubProcess.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.28" />' >> Process-SubProcess.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Process-SubProcess.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Process-SubProcess.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Process-SubProcess.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/Process/ChildProcess.pm' 'blib/lib/Process/ChildProcess.pm' \
	  'lib/Process/ChildProcessExecutable.pm' 'blib/lib/Process/ChildProcessExecutable.pm' \
	  'lib/Process/SubProcess.pm' 'blib/lib/Process/SubProcess.pm' \
	  'lib/Process/SubProcess/Group.pm' 'blib/lib/Process/SubProcess/Group.pm' \
	  'lib/Process/SubProcess/Pool.pm' 'blib/lib/Process/SubProcess/Pool.pm' 
