...
sub setMinProcessCount
{
  ...
  if(scalar(@_) > 0)
  {
    ...
    $self->{"_min_process_count"} = -1 unless($self->{"_min_process_count"} =~ /^-?\d+$/);
  }
  ...
  $self->{"_min_process_count"} = -1 unless(defined $self->{"_min_process_count"});
...
sub setMaxProcessCount
{
  ...
  if(scalar(@_) > 0)
  {
    ...
    #The Parameter is not a Number
    $self->{"_max_process_count"} = -1 unless($self->{"_max_process_count"} =~ /^-?\d+$/);
  }
  else  #No Parameters given
  {
  ...
  $self->{"_max_process_count"} = -1 unless(defined $self->{"_max_process_count"});
    ...
    if($irng == $icnt)
    {
        ...
        || $self->{"_execution_timeout"} > -1)
      {
        ...
        if($self->{"_debug"} > 0
          && $self->{"_quiet"} < 1)
        {
          print "wait tm chk: '$itmchk / " . $self->{"_check_interval"} . "'\n";
